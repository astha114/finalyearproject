...
@app.route("/predictor2/", methods=['POST', 'GET'])
def predictor2():
    s1=s2=s3=s4=s5=None
    if request.method == 'POST':
        s1 =  request.form['sone']
        s2 = request.form['stwo']
        s3 = request.form['sthree']
        s4 = request.form['sfour']
        s5 = request.form['sfive']
        
        ans = NaiveBayes(s1,s2,s3,s4,s5)
        summary = wikipedia.summary(ans, sentences = 4)
        return render_template('predictor2.html', ans=ans, summary=summary,s1=s1,s2=s2,s3=s3,s4=s4,s5=s5)
    else:
        if s1==None and s2==None and s3==None and s4==None and s5==None:
            return render_template('predictor2.html', ans='Enter Symptoms!')
        else:
